...
void stageController::createMacroFromCoordinates(std::string pathToCoordinates)
{
    ...
    auto storPos = std::vector<std::vector<double>>(0, std::vector<double>(3));

    std::fstream f;
    f << std::fixed;
    f << std::setprecision(5);

    f.open(pathToCoordinates);
    if(!(f.is_open()))
    {
        std::cout<<"trying to open"<< pathToCoordinates << " failed" <<std::endl;
    }else
    {
        std::cout<< pathToCoordinates << " opened " <<std::endl;
    ...
    for(int i = 0; i<storPos.size();i++){
        std::cout<<storPos[i][0]<<" "<<storPos[i][1]<<" "<<storPos[i][2]<<std::endl;
    }

    //////////////////////////////////////////
    //		Create the delay vector         //
    //////////////////////////////////////////
    ...
    //Close Shutter
    f << setLimitsMacro(1, 0, 0, 0, 200);
    f << "VEL A " << "9000" << " B " << "9000" << " C " << "9000" << std::endl;
    f << "MOV A " << 100 << " B " << 100<< " C " << 100 << std::endl;
    f << "DEL 1000" << std::endl;
    f << "MAC END" << std::endl;
    f.close();
